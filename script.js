...
function showToast(message) {
    ...
    toast.style = "position:fixed;bottom:30px;right:30px;background:#333;color:#fff;padding:10px 20px;border-radius:8px;z-index:10000;";
...
function setupGitHubProjects() {
    ...
    fetch(`https://api.github.com/users/${githubUsername}/repos`)
        ...
        .then(data => {
            allRepos = data;
            const uniqueLangs = new Set(["All", ...data.map(repo => repo.language).filter(Boolean)]);

            // ✅ Create filter buttons dynamically
            uniqueLangs.forEach(lang => {
                const btn = document.createElement("button");
                btn.textContent = `${lang} (${data.filter(repo => repo.language === lang).length || data.length})`;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (aboutSection) {
        aboutSection.addEventListener("mouseover", function handler() {
            aboutSection.innerHTML += "<p class='fun-fact'>Fun Fact: I built my first full-stack application at the age of 16! 🚀</p>";
...
console.log("%cYou found a hidden message! 🚀 Want to collaborate? Reach out at userparth@gmail.com!", "color: cyan; font-size: 16px;");